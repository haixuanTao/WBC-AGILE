...
_MJ_TO_SNAP_IDX = {
    "hipz_left":     0,  "hipx_left":     1,  "hipy_left":     2,
    "knee_left":     3,  "ankley_left":   4,  "anklex_left":   5,
    "hipz_right":    6,  "hipx_right":    7,  "hipy_right":    8,
    "knee_right":    9,  "ankley_right":  10, "anklex_right":  11,
}
...
def _build_init_from_state_log(csv_path: Path):
    ...
    joint_pos_rad = {
        mj: np.deg2rad(float(row[f"{csv}_pos_deg"])) for mj, csv in _MJ_TO_CSV.items()
    }
    joint_vel_rad = {
        mj: float(row[f"{csv}_vel_rad_s"]) for mj, csv in _MJ_TO_CSV.items()
    }
    ...
    print(f"[init-log] seeded from {csv_path} last row (t={row.get('time_s')}):"
...
def _build_init_from_snapshot(snapshot_path: Path):
    ...
    joint_pos_rad = {
        mj: np.deg2rad(float(jp_deg[idx])) for mj, idx in _MJ_TO_SNAP_IDX.items()
    }
    joint_vel_rad = {
        mj: float(jv_rad[idx]) for mj, idx in _MJ_TO_SNAP_IDX.items()
    }
    ...
    print(f"[init-snap] seeded from {snapshot_path} (t={snap.get('time_s')}): "
...
def main() -> int:
    ...
    ap.add_argument("--policy", type=Path, required=True,
                    help=".pt or .onnx — PolicyWrapper handles both")
    ...
    ap.add_argument("--init-from-state-log", type=Path, default=None,
                    help="Seed sim from the last row of this bipedal_state_log.csv")
    ap.add_argument("--init-from-snapshot", type=Path, default=None,
    ...
    config, sim, obs_proc, act_proc, cmd_prov = build_once(
        args.s2m_config, args.mjcf, device
    )
    init_fn = None
    ...
    try:
        result = run_eval(
            config=config, sim=sim, obs_processor=obs_proc,
            act_processor=act_proc, command_provider=cmd_prov,
            checkpoint=args.policy, out_dir=args.out_dir, device=device,
            fall_height_m=args.fall_height, init_fn=init_fn,
        )
    finally:
        sim.close()

    print(f"[sim_rollout] fall_rate={result.fall_rate:.2f}  "
          f"mean_survival={result.mean_survival_time_s:.1f}s  "
          f"mp4s -> {args.out_dir}")
    for r in result.rollouts:
        status = "FELL" if r.fell else "OK"
        print(f"    {r.name:10s}  survived={r.survival_time_s:5.1f}s  {status}  "
