...
Each axis belongs to one of four categories (see --list-axes for the menu):
...
Reuses the watcher's `_run_single_rollout` and matches its CLI conventions
for `--init-quat`, `--track-body`, `--fall-height`, and `--schedules`, so a
sweep can be configured against any robot the watcher already supports.
...
Examples:
    ...
    # Run all 12 axes with a custom robot orientation:
    python scripts/sim2mujoco_sweep.py --checkpoint <ckpt> --config <cfg> \
        --mjcf <scene.xml> --category all --output /tmp/all.json \
        --init-quat "0.9946 0.0523 0.0872 0.0" --track-body torso

    # Custom values on a single axis:
    python scripts/sim2mujoco_sweep.py --checkpoint <ckpt> --config <cfg> \
...
def init_orient_deg(
    ...
    *,
    ...
    init_quat_wxyz: tuple[float, float, float, float],
) -> float:
    """Initial-state init_fn: rotate the base by ``value`` degrees about ``axis``."""
    pelvis_z = _init_base_pose(sim, init_quat_wxyz=init_quat_wxyz)
...
def _wrap_init(
    ...
    def _fn(sim):
        pelvis_z = (
            axis_init(sim) if axis_init is not None else _init_base_pose(sim, init_quat_wxyz=init_quat_wxyz)
        )
        if noise_std > 0:
            n = sim.mj_model.nq - 7
...
def run_sweep(
    *,
    ...
    fall_height_m: float,
    init_quat_wxyz: tuple[float, float, float, float] = IDENTITY_QUAT,
    ...
    schedules = schedules if schedules is not None else SCHEDULES
            ...
            for value in values:
                ...
                for trial in range(n_trials):
                    for sn, sched in schedules.items():
                        ...
                        axis_init = (
                            (
                                lambda _sim, v=value, p=cfg.params, r=rng, fn=cfg.func, q=init_quat_wxyz: fn(
                                    _sim, float(v), rng=r, init_quat_wxyz=q, **p
                                )
                            )
                            if cfg.kind == "init"
                            else None
                        )
                        init_fn = _wrap_init(axis_init, rng, trial_joint_noise_std, init_quat_wxyz)
...
def main() -> None:
    ...
    p.add_argument(
        ...
        type=float,
        default=0.35,
        help="Pelvis-z below which a rollout counts as fallen; depends on the robot.",
    )
    p.add_argument(
        "--init-quat",
        type=_parse_quat,
        default=IDENTITY_QUAT,
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="YAML file overriding the watcher's built-in command schedules. "
        "Top-level mapping is name -> [[t, vx, vy, wz, height, label], ...].",
    )
    ...
    schedules = _load_schedules_yaml(args.schedules) if args.schedules is not None else None
