...
from agile.rl_env.assets.robots import lerobot_humanoid_no_arms as robot
...
class SceneCfg(InteractiveSceneCfg):
    ...
    # robots
    robot = robot.LEROBOT_NO_ARMS_DELAYED_DC_CFG.replace(prim_path="{ENV_REGEX_NS}/Robot")
    ...
    height_measurement_sensor = RayCasterCfg(
        prim_path="{ENV_REGEX_NS}/Robot/torso_subassembly",
...
class ActionsCfg:
    ...
    lift = mdp.LiftActionCfg(
        asset_name="robot",
        link_to_lift=robot.TRUNK_BODY,  # Head
        ...
        target_height=robot.DEFAULT_TRUNK_HEIGHT,
...
class RewardsCfg:
    ...
    joint_deviation_l1 = RewTerm(
        ...
        params={
            ...
            "standing_height_threshold": robot.DEFAULT_TRUNK_HEIGHT * 0.8,
            "sensor_cfg": SceneEntityCfg("height_measurement_sensor"),
            "mode": "l1",
        },
    )
    ...
    joint_deviation_l1_upper_body = RewTerm(
        ...
        params={
            "asset_cfg": SceneEntityCfg(
                "robot",
                joint_names=robot.ARM_JOINT_NAMES + robot.HEAD_JOINT_NAMES + robot.WAIST_JOINT_NAMES,
            ),
            "standing_height_threshold": robot.DEFAULT_TRUNK_HEIGHT * 0.8,
            "sensor_cfg": SceneEntityCfg("height_measurement_sensor"),
            "mode": "l1",
    ...
    orientation = RewTerm(
        ...
        weight=-5.0,
        params={"asset_cfg": SceneEntityCfg("robot", body_names=[robot.TRUNK_BODY])},
    )
    ...
    illegal_contacts = RewTerm(
        ...
        weight=-1.0,
        params={
            "sensor_cfg": SceneEntityCfg("contact_forces", body_names=robot.UNDESIRED_CONTACTS_LINKS),
            "threshold": 1.0,
        },
    )
    ...
    feet_distance = RewTerm(
        ...
        weight=-50.0,
        params={
            "asset_cfg": SceneEntityCfg("robot", body_names=robot.FEET_LINK_NAMES),
    ...
    feet_yaw_mean = RewTerm(
        ...
        weight=-5.0,
        params={
            "feet_asset_cfg": SceneEntityCfg("robot", body_names="foot_subassembly.*"),
            "base_body_cfg": SceneEntityCfg("robot", body_names=robot.TRUNK_BODY),
            "standing_height_threshold": robot.DEFAULT_TRUNK_HEIGHT * 0.8,
        },
    )
...
class TerminationsCfg:
    ...
    no_height_progress = DoneTermEx(
        ...
        sigma=1.0,
        params={
            "asset_cfg": SceneEntityCfg("robot", body_names=robot.TRUNK_BODY),
            "sensor_cfg": SceneEntityCfg("height_measurement_sensor"),
...
class EventCfg:
    ...
    # Per-joint-group DR for friction and armature, scales ported from the
    # mjlab lerobot velocity training config. Armature scales are narrowed
    # from the previous (0.0, 2.0) — the old lower bound at 0x was unphysical
    # (zero reflected rotor inertia). Friction scales are group-specific to
    # reflect that hips/knees/ankles wear and vary differently.
    randomize_joint_friction_hipz_hipx = EventTerm(
        func=mdp.randomize_joint_parameters,
        mode="startup",
        params={
            "asset_cfg": SceneEntityCfg("robot", joint_names=[".*hipz.*", ".*hipx.*"]),
            "friction_distribution_params": (0.6, 1.6),
            "operation": "scale",
            "distribution": "uniform",
        },
    )
    randomize_joint_friction_hipy_knee = EventTerm(
        func=mdp.randomize_joint_parameters,
        mode="startup",
        params={
            "asset_cfg": SceneEntityCfg("robot", joint_names=[".*hipy.*", ".*knee.*"]),
            "friction_distribution_params": (0.7, 1.5),
            "operation": "scale",
            "distribution": "uniform",
        },
    )
    randomize_joint_friction_ankle = EventTerm(
        ...
        mode="startup",
        params={
            "asset_cfg": SceneEntityCfg("robot", joint_names=[".*ankley.*", ".*anklex.*"]),
            "friction_distribution_params": (0.5, 2.0),
            "operation": "scale",
            "distribution": "uniform",
        },
    )
    randomize_joint_armature_hipz_hipx = EventTerm(
        func=mdp.randomize_joint_parameters,
        mode="startup",
        params={
            "asset_cfg": SceneEntityCfg("robot", joint_names=[".*hipz.*", ".*hipx.*"]),
    ...
    randomize_joint_armature_hipy_knee = EventTerm(
        func=mdp.randomize_joint_parameters,
        mode="startup",
        params={
            "asset_cfg": SceneEntityCfg("robot", joint_names=[".*hipy.*", ".*knee.*"]),
    ...
    randomize_joint_armature_ankle = EventTerm(
        func=mdp.randomize_joint_parameters,
        mode="startup",
        params={
            "asset_cfg": SceneEntityCfg("robot", joint_names=[".*ankley.*", ".*anklex.*"]),
    ...
    apply_external_force_torque_extremities = EventTerm(
        ...
        interval_range_s=(0.0, 10.0),
        params={
            "asset_cfg": SceneEntityCfg("robot", body_names=["foot_subassembly.*"]),
            "force_range": (-5.0, 5.0),
...
class CurriculumCfg:
    ...
    terrain_levels = CurrTerm(
        ...
        params={
            "min_height": robot.DEFAULT_TRUNK_HEIGHT * 0.8,
            "asset_cfg": SceneEntityCfg("robot"),
            "sensor_cfg": SceneEntityCfg("height_measurement_sensor"),
            "n_successes": 5,
            "n_failures": 5,
    ...
    adaptive_lift = CurrTerm(
        ...
        params={
            ...
            "standing_height_threshold": robot.DEFAULT_TRUNK_HEIGHT - 0.1,
...
@configclass
class LeRobotStandUpEnvCfg(ManagerBasedRLEnvCfg):
