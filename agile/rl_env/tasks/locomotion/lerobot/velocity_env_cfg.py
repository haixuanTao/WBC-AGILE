...
class MySceneCfg(InteractiveSceneCfg):
    ...
    height_measurement_sensor_left_foot = RayCasterCfg(
        prim_path="{ENV_REGEX_NS}/Robot/foot_subassembly",
    ...
    height_measurement_sensor_right_foot = RayCasterCfg(
        prim_path="{ENV_REGEX_NS}/Robot/foot_subassembly_2",
        offset=RayCasterCfg.OffsetCfg(pos=(0.01, 0.0, 1.0)),
        ray_alignment="yaw",
        pattern_cfg=patterns.GridPatternCfg(resolution=0.05, size=(0.2, 0.1)),
        debug_vis=False,
...
class ActionsCfg:
    ...
    joint_pos = mdp.JointPositionActionCfg(
        asset_name="robot",
        joint_names=robot.LEG_JOINT_NAMES,
        scale=1.0,
        use_default_offset=True,
        clip={".*": (-1.0, 1.0)},
    )

    # random_pos for upper-body joints — disabled for LeRobot no-arms (no upper body).
...
class RewardsCfg:
    ...
    track_ang_vel = RewTerm(
        ...
        params={
            ...
            "std": 0.2,
            "asset_cfg": SceneEntityCfg("robot", body_names=["torso_subassembly"]),
        },
    )
    ...
    feet_slip = RewTerm(
        ...
        params={
            ...
            "sensor_cfg": SceneEntityCfg("contact_forces", body_names="foot_subassembly.*"),
            "robot_cfg": SceneEntityCfg("robot", body_names="foot_subassembly.*"),
        },
    )
    ...
    feet_roll = RewTerm(
        ...
        weight=-0.01,
        params={"asset_cfg": SceneEntityCfg("robot", body_names="foot_subassembly.*")},
    )
    ...
    feet_yaw_mean = RewTerm(
        ...
        params={
            ...
            "base_body_cfg": SceneEntityCfg("robot", body_names="torso_subassembly"),
        },
    )
...
class TerminationsCfg:
    ...
    illegal_base_height = DoneTerm(
        ...
        params={
            "asset_cfg": SceneEntityCfg("robot", body_names="torso_subassembly"),
            "sensor_cfg": SceneEntityCfg("height_measurement_sensor"),
            "height_threshold": robot.DEFAULT_TRUNK_HEIGHT - 0.3,
        },
    )
...
class LocomotionEventCfg:
    ...
    # Consolidated whole-robot DR (AGILE's randomize_joint_parameters doesn't
    # support joint subsetting — it errors on non-identity joint_ids).
    # Ranges widened vs T1 to match the per-group spread we wanted.
...
@configclass
class LeRobotVelocityEnvCfg(ManagerBasedRLEnvCfg):
